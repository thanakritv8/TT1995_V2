...
var columnHide = [];
var gbTableId = '34';
var tableName = "driving_license_natural_gas_transportation";
var idFile;
var _dataSource;
var gbE;
//คลิกขวาโชว์รายการ   
...
$(function () {
    $("a:contains('บอข. ขนส่งก๊าสธรรมชาติ')").first().addClass("active");
    ...
    var cf = $(".custom-file").dxFileUploader({
        ...
        onValueChanged: function (e) {
            ...
            if (files.length > 0) {
                ...
                fileDataPdf.append('fk_id', gbE.currentSelectedRowKeys[0].dlngt_id);
            }
        },
    }).dxFileUploader('instance');
    //จบการกำหนด Upload files

    function getDataDLNGT() {
        var dataValue = [];
        //โชว์ข้อมูลทะเบียนทั้งหมดใน datagrid
        return $.ajax({
            type: "POST",
            url: "../Home/GetDLNGTData",
    ...
    var dataGrid = $("#gridContainer").dxDataGrid({
        ...
        onRowUpdating: function (e) {
            if (!fnUpdateDLNGT(e.newData, e.key.dlngt_id)) {
                e.cancel = true;
            }
            setDefaultLookupDriver();
        },
        onRowInserting: function (e) {
            var idInsert = fnInsertDLNGT(e.data);
            if (idInsert != 0) {
                e.data.dlngt_id = idInsert;
        ...
        onRowRemoving: function (e) {
            e.cancel = fnDeleteDLNGT(e.key.dlngt_id);
        },
        masterDetail: {
            enabled: false,
                        ...
                        $('<a style="color:green;font-weight:bold;" />').addClass('dx-link')
                            ...
                            .on('dxclick', function (e) {
                                ...
                                var gridHistory = $("#gridHistory").dxDataGrid({
                                    dataSource: fnGetHistory(gbTableId, options.row.data.dlngt_id),
                                    showBorders: true,
                                    height: 'auto',
                                    scrolling: {
                                        mode: "virtual"
    ...
    function fnInsertDLNGT(dataGrid) {
        dataGrid.IdTable = gbTableId;
        var returnId = 0;
        $.ajax({
            type: "POST",
            url: "../Home/InsertDLNGT",
            contentType: "application/json; charset=utf-8",
    ...
    function fnDeleteDLNGT(keyItem) {
        var returnStatus;
        $.ajax({
            type: "POST",
            url: "../Home/DeleteDLNGT",
            contentType: "application/json; charset=utf-8",
            data: "{keyId: '" + keyItem + "'}",
            dataType: "json",
            async: false,
    ...
    function fnUpdateDLNGT(newData, keyItem) {
        //console.log(keyItem);
        newData.key = keyItem;
