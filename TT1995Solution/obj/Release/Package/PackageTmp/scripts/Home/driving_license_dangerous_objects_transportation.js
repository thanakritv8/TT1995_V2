...
var columnHide = [];
var gbTableId = '35';
var tableName = "driving_license_dangerous_objects_transportation";
var idFile;
var _dataSource;
var gbE;
//คลิกขวาโชว์รายการ   
...
$(function () {
    $("a:contains('บอข. ขนส่งวัตถุอันตราย')").first().addClass("active");
    ...
    var cf = $(".custom-file").dxFileUploader({
        ...
        onValueChanged: function (e) {
            ...
            if (files.length > 0) {
                ...
                fileDataPdf.append('fk_id', gbE.currentSelectedRowKeys[0].dldot_id);
            }
        },
    }).dxFileUploader('instance');
    //จบการกำหนด Upload files

    function getDataDLDOT() {
        var dataValue = [];
        //โชว์ข้อมูลทะเบียนทั้งหมดใน datagrid
        return $.ajax({
            type: "POST",
            url: "../Home/GetDLDOTData",
    ...
    var dataGrid = $("#gridContainer").dxDataGrid({
        ...
        onRowUpdating: function (e) {
            if (!fnUpdateDLDOT(e.newData, e.key.dldot_id)) {
                e.cancel = true;
            }
            setDefaultLookupDriver();
        },
        onRowInserting: function (e) {
            var idInsert = fnInsertDLDOT(e.data);
            if (idInsert != 0) {
                e.data.dldot_id = idInsert;
        ...
        onRowRemoving: function (e) {
            e.cancel = fnDeleteDLDOT(e.key.dldot_id);
        },
        masterDetail: {
            enabled: false,
                        ...
                        $('<a style="color:green;font-weight:bold;" />').addClass('dx-link')
                            ...
                            .on('dxclick', function (e) {
                                ...
                                var gridHistory = $("#gridHistory").dxDataGrid({
                                    dataSource: fnGetHistory(gbTableId, options.row.data.dldot_id),
                                    showBorders: true,
                                    height: 'auto',
                                    scrolling: {
                                        mode: "virtual"
    ...
    function fnInsertDLDOT(dataGrid) {
        dataGrid.IdTable = gbTableId;
        var returnId = 0;
        $.ajax({
            type: "POST",
            url: "../Home/InsertDLDOT",
            contentType: "application/json; charset=utf-8",
    ...
    function fnDeleteDLDOT(keyItem) {
        var returnStatus;
        $.ajax({
            type: "POST",
            url: "../Home/DeleteDLDOT",
            contentType: "application/json; charset=utf-8",
            data: "{keyId: '" + keyItem + "'}",
            dataType: "json",
            async: false,
    ...
    function fnUpdateDLDOT(newData, keyItem) {
        //console.log(keyItem);
        newData.key = keyItem;
        ...
        dataGrid.option('dataSource', getDataDLDOT());
        dataGrid.refresh();
        return returnStatus;
    }
